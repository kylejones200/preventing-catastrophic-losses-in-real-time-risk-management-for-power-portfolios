...
def visualize_results(
    mtl_results, single_results, y_test, corr_matrix, plot: bool = False
):
    ...
    if plot:
        ...
        improvements = [
            (single_maes[i] - mtl_maes[i]) / single_maes[i] * 100
        ...
        for idx, (task, task_name) in enumerate(zip(tasks, task_names)):
            ...
            ax.scatter(
                actual, mtl_pred, alpha=0.5, s=30, edgecolors="none", color="#3498db"
            )
            ...
            ax.set_ylabel(
                f"Predicted {task_name} (log)", fontweight="bold", fontsize=10
            )
            ax.set_title(
                f"{task_name} Predictions (R²={r2:.3f})", fontweight="bold", fontsize=11
            )
...
def main():
    ...
    for task, name in [("co2", "CO₂"), ("nox", "NOx"), ("so2", "SO₂")]:
        ...
        logger.info(
            f"{name:<10} {single_mae:<20.4f} {mtl_mae:<20.4f} {improvement:+.1f}%"
        )
    ...
    logger.info(
        f"{'Average':<10} {avg_single:<20.4f} {avg_mtl:<20.4f} {avg_improvement:+.1f}%"
    )
