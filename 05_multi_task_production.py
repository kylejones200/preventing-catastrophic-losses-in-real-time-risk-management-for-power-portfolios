...
class _MLPForecaster(nn.Module):
    ...
    def __init__(self, n_features: int, output_size: int=1):
        super().__init__()
        self.net = nn.Sequential(nn.Flatten(), nn.LazyLinear(128), nn.ReLU(), nn.Linear(128, 64), nn.ReLU(), nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 16), nn.ReLU(), nn.Linear(16, 1), nn.ReLU(), nn.Linear(1, 16), nn.ReLU(), nn.Linear(16, 1), nn.ReLU(), nn.Linear(1, 16), nn.ReLU(), nn.Linear(16, 1), nn.ReLU(), nn.Linear(1, 128), nn.ReLU(), nn.Linear(128, 64), nn.ReLU(), nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 16), nn.ReLU(), nn.Linear(16, 1))
...
def train_mtl_model(X_train, X_test, y_train, y_test):
    ...
    history = _train_torch(model, X_train, {'co2_output': y_train['co2'], 'nox_output': y_train['nox'], 'so2_output': y_train['so2']})
    ...
    return {'model': model, 'history': history, 'predictions': {'co2': y_pred_co2.flatten(), 'nox': y_pred_nox.flatten(), 'so2': y_pred_so2.flatten()}, 'mae': {'co2': mae_co2, 'nox': mae_nox, 'so2': mae_so2}}
...
def visualize_results(mtl_results, single_results, y_test, corr_matrix, plot: bool=False):
    ...
    if plot:
        ...
        for i in range(3):
            for j in range(3):
                ax1.text(j, i, f'{corr_matrix.iloc[i, j]:.2f}', ha='center', va='center', color='black', fontsize=12, fontweight='bold')
        ...
        improvements = [(single_maes[i] - mtl_maes[i]) / single_maes[i] * 100 for i in range(len(tasks))]
        x = np.arange(len(task_names))
        width = 0.35
        ax2.bar(x - width / 2, single_maes, width, label='Single-Task', color='#e74c3c', alpha=0.8, edgecolor='black', linewidth=1.5)
        bars2 = ax2.bar(x + width / 2, mtl_maes, width, label='Multi-Task', color='#2ecc71', alpha=0.8, edgecolor='black', linewidth=1.5)
        ...
        for i, (bar, imp) in enumerate(zip(bars2, improvements)):
            ...
            ax2.text(bar.get_x() + bar.get_width() / 2.0, height, f'+{imp:.1f}%', ha='center', va='bottom', fontsize=10, fontweight='bold', color='green')
        ...
        for idx, (task, task_name) in enumerate(zip(tasks, task_names)):
            ...
            ax.hist(single_residuals, bins=30, alpha=0.7, color='#e74c3c', label='Single-Task', edgecolor='black')
            ax.hist(mtl_residuals, bins=30, alpha=0.7, color='#2ecc71', label='Multi-Task', edgecolor='black')
            ...
            ax.legend(fontsize=9)
        plt.suptitle('Multi-Task Learning: Simultaneous Prediction of Three Pollutants', fontsize=16, fontweight='bold', y=0.995)
...
def main_step_001() -> None:
    ...
    feature_cols = ['capacity_mw', 'generation_mwh', 'heat_input_mmbtu', 'capacity_factor', 'heat_rate']
    target_cols = ['log_co2', 'log_nox', 'log_so2']
    data = df[feature_cols + target_cols].dropna()
    logger.info(f'\nTraining on {len(data):,} plants with complete data')
    X = data[feature_cols]
    y = {'co2': data['log_co2'].values, 'nox': data['log_nox'].values, 'so2': data['log_so2'].values}
    corr_matrix = analyze_correlations(data, target_cols)
    X_train, X_test, y_co2_train, y_co2_test = train_test_split(X, y['co2'], test_size=TEST_SIZE, random_state=RANDOM_STATE)
    _, _, y_nox_train, y_nox_test = train_test_split(X, y['nox'], test_size=TEST_SIZE, random_state=RANDOM_STATE)

def main_step_002() -> None:
    global X, X_test, X_train, corr_matrix, mtl_results, scaler, single_results, y, y_co2_test, y_co2_train, y_nox_test, y_nox_train
    _, _, y_so2_train, y_so2_test = train_test_split(X, y['so2'], test_size=TEST_SIZE, random_state=RANDOM_STATE)
